...
def analyze_instructions(user_name, invest_type, interest, today_date=today_date):
    # ETF
    ETF = [
        "SPY",
        "QQQ",
    ...
    오늘 날짜는 {today_date}야.\
    고객이 현재 투자하고 있는 ETF는 {interst_ETF}가 있어.\
    너가 주의 깊게 실시간으로 봐야하는 ETF는 {ETF}야.\
    너의 답변은 너무 길어서는 안돼. 예를 들어, '{user_name} 고객님, OOO ETF가 앞으로 전망이 긍정적으로 보입니다. 투자 금액을 소폭 올려 보는건 어떠신가요?'와 같아."